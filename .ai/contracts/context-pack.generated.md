...
- Active initiative: WiFi voucher sales MVP for Saleslip (`docs/wifi-platform-plan.md`)
...
| 2026-09-24 | Dashboard overview on real hotspot metrics | `apps/web/src/components/dashboard/overview-view.tsx`, `overview-stat-row.tsx` (new), `wifi/wifi-sessions-table.tsx`, `nav-config.ts`, `page-title.tsx`, `search-command.tsx`, `app-sidebar.tsx`, `apps/web/src/app/dashboard/[section]/` (deleted), `packages/api/src/wifi/repository.ts`, `packages/api/src/router/wifi.ts`, `packages/api/src/router/wifi-router.ts`, `packages/api/src/__tests__/wifi.test.ts` (new), `packages/api/src/__tests__/wifi-router.test.ts` | Overview = four hero `StatCard`s (connected now, revenue today, vouchers generated, active vouchers) + the live sessions table with a new Paid column. API is additive: `/stats` gains `generated {total, today}`; `/sessions` rows gain `amountKobo` (null for counter vouchers). Connected-now degrades to a dash with a router-unreachable note instead of failing. Template components deleted and nav pruned to sections that exist; the `[section]` catch-all was removed because Cache Components forbids an empty `generateStaticParams`. Spec: `.ai/specs/active/dashboard-overview-metrics.spec.md`. |
...
- Do not import `@turbo/wifi`'s barrel from `"use client"` files; `plans.ts`
  imports node-only `@turbo/routeros`. Use the `@turbo/wifi/format` subpath.
...
| `@turbo/wifi` | `packages/wifi` | `@turbo/db`, `@turbo/eslint-config`, `@turbo/prettier-config`, `@turbo/routeros`, `@turbo/tsconfig` |
